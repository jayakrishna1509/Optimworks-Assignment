...
export const adminLogin = async (req, res) => {
  try {
    const { email, password } = req.body; // Changed from adminid to email

    const admin = await adminModel.findOne({ email }); // Change field in query
    if (!admin) {
    ...
    const token = jwt.sign({ adminid: admin._id }, process.env.LOGIN_SECRET_KEY);
    res.status(200).json({ 
      success: true, 
